...
let logged_raw = ref 0;;
let exported_theories = ref 0;;
let logged_thms = ref 0;;
let logged_namedthms = ref 0;;
let logged_tms = ref 0;;
...
let log_raw s =
    match (!log_state) with
      Active_logging (h,_) ->
		let () = (logged_raw := !logged_raw +1) in output_string h (s ^ "\n")
    | _ -> ();;
...
let (log_term,log_proof,log_clear) =
    ...
    and log_term tm =
        ...
        let () = (logged_tms := !logged_tms +1) in
        if saved ob then () else
        let () =
            if is_const tm then
    ...
    and log_proof th =
        ...
        let () =
            ...
            | Axiom_proof (Some s) ->
                let () = log_list log_term (hyp th) in
                let () = log_term (concl th) in
                let () = log_name (Name.mk_global s) in
                let () = log_command "namedAxiom" in
                ()
            | Refl_proof tm ->
                let () = log_term tm in
                let () = log_command "refl" in
            ...
            | Define_const_list_proof c ->
                (match (peek_const_def c) with
                   Some (Const_list_definition cdef) ->
                     ...
                     ()
                 | _ ->
                   failwith ("thm out of const list definition scope: " ^ c))
			| Conj_proof (th1,th2) ->
                let () = log_proof th2 in
                let () = log_proof th1 in
                let () = log_command "conj" in
                ()
            | Conjunct1_proof th ->
                let () = log_proof th in
                let () = log_command "conjunct1" in
                ()
            | Conjunct2_proof th ->
                let () = log_proof th in
                let () = log_command "conjunct2" in
                ()
			| Truth_proof ->
                let () = log_command "truth" in
                ()
            | Noti_proof th ->
                let () = log_proof th in
                let () = log_command "noti" in
                ()
            | Note_proof th ->
                let () = log_proof th in
                let () = log_command "note" in
                ()
            | Contr_proof (th,a) ->
                let () = log_proof th in
                let () = log_term a in
                let () = log_command "contr" in
                ()
            | Mp_proof (th1,th2) ->
                let () = log_proof th2 in
                let () = log_proof th1 in
            ...
            | Disj1_proof (th,a) ->
                let () = log_proof th in
                let () = log_term a in
                let () = log_command "disj1" in
                ()
            | Disj2_proof (th,a) ->
                let () = log_proof th in
                let () = log_term a in
                let () = log_command "disj2" in
                ()
            | Disjcases_proof (th1,th2,th3) ->
                let () = log_proof th1 in
                let () = log_proof th2 in
                let () = log_proof th3 in
                let () = log_command "disjcases" in
                ()
            | Gen_proof (th,x) ->
                let () = log_proof th in
                let () = log_term x in
            ...
            | Spec_proof (s,th) ->
                ...
                ()
            | Exists_proof (etm,y,th) ->
                let () = log_term etm in
                let () = log_term y in
                let () = log_proof th in
                let () = log_command "exists" in
                ()
            | Choose_proof (v,th1,th2) ->
                let () = log_term v in
                let () = log_proof th1 in
                let () = log_proof th2 in
                let () = log_command "choose" in
                ()
		in		
        let _ = save_top_obj ob in
        ()
    and log_object obj =
        ...
        | Object.Thm_object t -> log_proof t
        | Object.Namedthm_object (_,t) -> log_proof t in
...
let log_thm th =
    ...
    let () = (logged_thms := !logged_thms +1) in
    ();;

let log_namedthm th s =
    let () = log_proof th in
    let () = log_list log_term (hyp th) in
    let () = log_term (concl th) in
    let () = log_name (Name.mk_global s) in
    let () = log_command "namedThm" in
    let () = (logged_namedthms := !logged_namedthms +1) in
    ();;
...
let logfile_end () =
    ...
    match (!log_state) with
      Active_logging (h,_) ->
        ...
        let () = (exported_theories := !exported_theories +1) in
        ()
    | _ -> ();;
...
let the_exported_thms =
    ref (Sequent_map.empty : (thm * (string option * string)) Sequent_map.t);;

let add_the_exported_thms th so thy =
    the_exported_thms := add_sequent_map (!the_exported_thms) (th,(so,thy));;
...
let add_exported_thm thy th =
    let () = delete_proof th None in
    let () = delete_type_op_definition (thm_type_ops th) in
    let () = delete_const_definition (thm_consts th) in
    let () = add_the_exported_thms th None thy in
    ();;
    
let add_exported_namedthm thy s th =
    let () = delete_proof th (Some s) in
    let () = delete_type_op_definition (thm_type_ops th) in
    let () = delete_const_definition (thm_consts th) in
    let () = add_the_exported_thms th (Some s) thy in
    ();;
...
and export_namedthm = Export.export_namedthm
