...
let compare =
  let rec cmp obj1 obj2 =
      ...
      | (Thm_object _, _) -> -1
      | (_,Thm_object _) -> 1
      | (Namedthm_object (_,t1), Namedthm_object (_,t2)) ->
        let c = Pervasives.compare (concl t1) (concl t2) in
        if c <> 0 then c else
        Pervasives.compare (hyp t1) (hyp t2)
...
let mk_namedthm s t = Namedthm_object (s,t);;

let dest_namedthm obj =
    match obj with
      Namedthm_object p -> p
    | _ -> failwith "Object.dest_namedthm";;
